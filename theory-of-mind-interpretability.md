...
A common critique of LLMs is that they rely purely on formal linguistic competence for predictions and therefore can't truly "learn" meaning in a deep sense<sub>[<a href="https://aclanthology.org/2020.acl-main.463.pdf" title="Bender" rel="nofollow">18</a>]</sub>. But if we zoom in on emergent understanding—the idea that models develop an implicit sense of meaning based on patterns and correlations in the data—an intuitive question to ask would be: How do pattern-matching mechanisms like induction heads manage to capture semantics effectively enough to succeed on ToM tasks? 

One plausible hypothesis is that while induction heads are primarily focused on formal patterns, the semantic meaning embedded in those patterns gets absorbed through the model’s training process. For instance, repeated references to "the cat being on the basket" provide a robust contextual anchor. Even though induction heads are just tracking sequence-level correlations, these correlations are tied to real-world semantics in the training data. So when a model predicts that “the cat is in the basket”, it might actually be leveraging a weakly implicit form of semantic understanding (functional competency) encoded in its layers.

This idea becomes more compelling when you consider tasks where models are trained to predict mental states or perspectives. Even if the model starts by exploiting high-level patterns in the data, those patterns are often tightly aligned with semantic reasoning. For example, layers deeper in the network—say, layer 22—don’t just pass through raw pattern data from earlier layers. Instead, they integrate signals representing a mix of formal linguistic structure and contextual cues. By this point, the model might be blending purely formal reasoning with the semantic relationships encoded in the data.

This raises a key question: When the model predicts John’s perspective in a ToM task, is it actually reasoning about John’s mental state (functional competence)? Or is it just leveraging high-level linguistic correlations (formal competence) that happen to align with correct answers? My bet is that there’s a blurry line here—meaning can emerge from form when it’s augmented by structured, implicit grounding in the data.

Take induction heads as an example. They’re not explicitly designed to handle grounded semantic signals, but they might implicitly leverage weak forms of grounding if the training data embeds consistent patterns that correlate with meaning. For instance, if the model sees "John thinks the cat is in the basket" paired repeatedly with specific outcomes, it might learn to associate those patterns with semantic relationships. By the time layer 22 comes into play, earlier layers have already encoded semantic cues into their representations, which the deeper layers can recombine into contextually grounded predictions.

Even without explicit grounding, models trained on structured datasets can still encode weak semantic signals. For example, datasets like MMLU or ARC-C embed linguistic patterns that implicitly carry semantic entailments or logical structures. Models like Gemma-2-2B, with their depth and sophisticated architecture, seem to capture these relationships effectively, even if they’re operating formally. By layer 22, relational data from earlier layers is synthesized into outputs that mimic semantic understanding.
