...
Ablation studies, component-level patching, and Q/K/V interventions demonstrate that certain attention heads form a compact ToM circuit. When intact, this circuit enables the model to track conflicting beliefs and produce correct answers. When you systematically remove or alter key heads, the model struggles, mirroring what happens if a human lacked the linguistic capabilities to represent other minds’ beliefs. 
...
- Is robust to targeted ablations. Critical heads responsible for ToM capabilities were isolated to validate the circuit, and the observed performance degradations following ablations affirm the importance of these components in maintaining robust ToM functionality.
    ...
    - Furthermore, experiments show that when certain tokens (those involved in representing “belief states” like where John thinks the cat is) are patched from a clean run, the corrupted model’s performance on a ToM-like task recovers.
...
- Weakly shows that as LLMs scale and learn dense correlations, they develop partial semantic grounding—patterns that mimic genuine semantic and pragmatic reasoning.
      - The ToM circuit appear to track particular tokens (like “basket” vs. “box”) consistently, carrying forward these representations across layers and contributing to final predictions. This suggests the model is doing more than superficial form matching; it’s maintaining stable semantic relations that resemble an understanding of the narrative. Pretty speculative, but I think the experiments lend credence by pinpointing heads whose removal or modification affects semantic coherence. While it’s not definitive proof that the model truly “understands” semantics, it’s a concrete demonstration that formal pattern capture is sufficient to manifest in behaviors strongly associated with semantic interpretation.
