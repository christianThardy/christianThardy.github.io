...
But there are tons of caveats here. I can kind of see what might be happening, but why this happens, in context, needs further investigation. Furthermore, in practice, a negative QK dot product alone does not always guarantee the model will focus positively on the “next best option.” Other tokens might also have low or modest QK alignment, so the end result depends on relative alignment scores across all tokens. 
