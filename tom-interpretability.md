...
- **Finding:** A 28-head circuit (~16% of total heads) recovers full ToM task performance; mean-ablating it drops the believed-vs-actual logit difference by ~80.66%.
    - The circuit decomposes as **duplicate-token → induction → copy-suppression → late previous-token** heads, with copy suppression carrying the heaviest load.
      - Ablating mid-layer suppression heads (~3% of total heads) alone drops performance by ~45%.
